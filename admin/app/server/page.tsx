  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4">
        <label className="text-sm font-medium text-gray-200">Server URL</label>
        <div className="mt-2 flex flex-col gap-3 sm:flex-row">
          <input
            value={value}
            onChange={(e) => setValue(e.target.value)}
            placeholder="http://192.168.0.6:8080"
            className="w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-sm text-white outline-none focus:border-indigo-500/50"
          />
          <button
            onClick={() => testConnection(value)}
            disabled={status === 'testing'}
            className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white hover:bg-white/10 disabled:opacity-60"
          >
