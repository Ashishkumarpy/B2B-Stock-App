...
function parseArgs(argv) {
  const args = {
    xlsx: 'D:\\Editing\\Done\\SIPPERS BOTTLE\\WATER BOTTLES.xlsx',
    imagesDir: 'D:\\Editing\\Done\\SIPPERS BOTTLE\\iMAGES\\No Logo',
    category: 'Water Bottles',
    headerRow: 3,
    cloudinaryPublicIdPrefix: 'water-bottles/no-logo',
    dryRun: false
  };
...
async function main() {
  ...
  for (let r = headerIdx + 1; r < rows.length; r++) {
    ...
    const imageFile = `WB-${pad2(idx)}.jpg`;
  ...
  const outPath = path.resolve(process.cwd(), 'water_bottles_import_results.json');
