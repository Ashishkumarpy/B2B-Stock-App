...
function parseArgs(argv) {
  const args = {
    imagesDir: 'D:\\Editing\\Done\\ID Cards\\New\\raw\\Catalogues',
    category: 'ID Cards',
    cloudinaryPublicIdPrefix: 'id-cards/no-logo',
    defaultPrice: 0,
    dryRun: false
  };
...
function codeFromFile(filename) {
  ...
  if (!m) return null;
  return `IDCARD-${String(Number(m[1])).padStart(2, '0')}`;
}
...
async function main() {
  ...
  const outPath = path.resolve(process.cwd(), 'id_cards_import_results.json');
