...
function parseArgs(argv) {
  const args = {
    xlsx: 'D:\\Editing\\Done\\Card Holder\\VC CARD.xlsx',
    imagesDir: 'D:\\Editing\\Done\\Card Holder\\Images\\No Logo',
    category: 'Card Holders',
    headerRow: 1,
    cloudinaryPublicIdPrefix: 'card-holders/no-logo',
    dryRun: false
  };
...
function normalizeCode(code) {
  // Example: "XG-VC-07 (PRO)" -> "XG-VC-07(PRO)"
  return String(code).trim().replace(/\s*\(\s*/g, '(').replace(/\s*\)\s*/g, ')').replace(/\s+/g, ' ');
}
...
async function main() {
  ...
  const outPath = path.resolve(process.cwd(), 'card_holders_import_results.json');
