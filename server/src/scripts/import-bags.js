...
function parseArgs(argv) {
  const args = {
    xlsx: 'D:\\Editing\\Done\\Bags\\Bags Price list.xlsx',
    imagesDir: 'D:\\Editing\\Done\\Bags\\No Logo',
    category: 'Bags',
    headerRow: null,
    cloudinaryPublicIdPrefix: 'bags/no-logo',
    dryRun: false
  };
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === '--header-row') args.headerRow = Number(argv[++i]); // optional override
...
async function main() {
  ...
  let headerIdx = args.headerRow ? args.headerRow - 1 : -1;
  if (headerIdx < 0) {
    headerIdx = rows.findIndex((r) =>
      Array.isArray(r) && r.some((v) => String(v || '').toUpperCase().includes('PRODUCT CODE'))
    );
  }
  if (headerIdx < 0) throw new Error('header row not found (expected PRODUCT CODE)');
  ...
  for (let r = headerIdx + 1; r < rows.length; r++) {
    ...
    const imageFile = `${codeStr}(No Logo).jpg`;
  ...
  const outPath = path.resolve(process.cwd(), 'bags_import_results.json');
