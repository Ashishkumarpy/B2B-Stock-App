...
function parseArgs(argv) {
  const args = {
    xlsx: "D:\\Editing\\Done\\yoyo's\\YOYO PRICES  XG AUG 25.xlsx",
    imagesDir: "D:\\Editing\\Done\\yoyo's\\Images",
    category: "Yoyo's",
    headerRow: 2,
    cloudinaryPublicIdPrefix: 'yoyos/no-logo',
    dryRun: false
  };
...
function yoyoFileFromCode(code) {
  // "XG-Y1" -> "XG-Y01(No Logo).jpg"
  const m = String(code).trim().match(/XG-Y(\d+)$/i);
  if (!m) return null;
  const n = String(m[1]).padStart(2, '0');
  return `XG-Y${n}(No Logo).jpg`;
}
...
async function main() {
  ...
  for (let r = headerIdx + 1; r < rows.length; r++) {
    ...
    const imageFile = yoyoFileFromCode(codeStr);
    if (!imageFile) {
  ...
  const outPath = path.resolve(process.cwd(), 'yoyos_import_results.json');
